...
<div class="flex h-full flex-1 flex-col items-center justify-center">
    <!-- Result Card -->
    <div class="w-full max-w-4xl bg-white/30 dark:bg-neutral-800/30 backdrop-blur-lg border border-white/40 dark:border-white/10 p-8 rounded-xl shadow-md">

        <!-- Header -->
        <div class="text-center mb-8">
            <svg class="h-20 w-20 text-primary mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <!-- Detailed Answers -->
        <div class="mb-8">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">Rincian Jawaban Anda</h2>
            <div class="border border-gray-200/80 dark:border-neutral-700/80 rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200/80 dark:divide-neutral-700/80">
                    <thead class="bg-gray-50/50 dark:bg-neutral-800/50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-neutral-400 uppercase tracking-wider">Pertanyaan</th>
                            <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-neutral-400 uppercase tracking-wider">Jawaban</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white/50 dark:bg-neutral-900/50 divide-y divide-gray-200/50 dark:divide-neutral-700/50">
                        @if(!empty($answers))
                            @foreach($answers as $step => $answer)
                                <tr>
                                    <td class="px-6 py-4 whitespace-normal text-sm text-gray-800 dark:text-neutral-200">{{ $this->getQuestionText($step) }}</td>
                        ...
                        @else
                            <tr>
                                <td colspan="2" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-neutral-400">Tidak ada data jawaban.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
        ...
        <!-- Action Buttons -->
        <div class="flex flex-col sm:flex-row justify-center items-center gap-4 pt-6 border-t border-gray-200/80 dark:border-neutral-700/80">
            ...
            <x-button-ghost-lg href="{{ route('konsultasi.starter') }}" wire:navigate>
                Ulangi Konsultasi
            </x-button-ghost-lg>
        </div>
